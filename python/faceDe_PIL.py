...
# the class to detect the face without using the opencv module
...
class faceDetect:
    ...
    _red=(0,0,255)
    _white=(255,255,255)
    _font=ImageFont.truetype('fontx/hwst.ttf',10)
    _name='UnknownPerson'
    ...
    def Detect(self,checkframe=''):
        ...
        if self.faces!='':
            ...
            for (top,right,bottom,left), cf_encoded in zip(cf_locations,cf_encodings):
                ...
                draw=ImageDraw.Draw(checkframe)
                draw.text((left,bottom-20),self._name,font=self._font,fill=self._red)
        return checkframe
